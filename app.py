...
st.title("🧠 DHARINI Multi-Specimen 3D Navigator")
st.caption("An interactive multi-planar fly-through engine powered by AWS Open Data.")
...
def load_and_map_specimen(model_name):
    ...
    if not os.path.exists(raw_path):
        ...
        with st.spinner(f"📥 Syncing {model_name} from AWS Registry (approx. 105MB)..."):
...
try:
    ...
    loop_speed = st.sidebar.slider("Playback Speed Interval (Seconds)", 0.01, 0.5, 0.05, step=0.01)

    # Secondary Manual Navigation Sliders
    ...
    # If the user toggles play, we hijack the rendering window and loop through 40 middle slices
    if st.session_state.play_loop:
        center_slice = max_slices // 2
        start_frame = max(0, center_slice - 20)
        end_frame = min(max_slices, center_slice + 20)
        
        # Continuous iteration frame loop container
        frame_placeholder = st.empty()
        
        while st.session_state.play_loop:
            for current_z in range(start_frame, end_frame):
                # Check if user pressed pause during iteration cycle
                ...
                # Write frame to web container viewport instantly
